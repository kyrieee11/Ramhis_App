...
app.get('/admin/dashboard', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const totalDoctors = await usersCol().countDocuments({
      account_type: 'doctor',
    });
    const totalVolunteers = await usersCol().countDocuments({
      $or: [{ account_type: 'volunteer' }, { account_type: 'user' }],
    });
    const pendingVerification = await usersCol().countDocuments({
      status: 'pending',
    });

    const recentUsersRaw = await usersCol()
      .find({})
      .sort({ created_at: -1 })
      .limit(5)
      .toArray();
    ...
    const recentUsers = recentUsersRaw.map((user) => ({
      name: normalizeFullName(user),
      role: user.account_type || user.role || 'user',
      status: user.status || 'active',
    ...
    const upcomingEventsRaw = await eventsCol()
      .find({})
      .sort({ created_at: -1 })
      .limit(5)
      .toArray();
